...
def _resolve_writable_log_dir() -> Path:
    """로컬은 프로젝트 logs/, Streamlit Cloud 등은 /tmp 하위(앱 디렉터리는 읽기 전용)."""
    for base in (ROOT / "logs", Path(tempfile.gettempdir()) / "multi_session_rag_logs"):
        try:
...
LOG_DIR = _resolve_writable_log_dir()
...
def process_uploaded_pdfs(
    ...
    for uf in files:
        ...
        with st.spinner(f"처리 중: {base}"):
            fd, tmp_path = tempfile.mkstemp(suffix=".pdf", prefix="multi_session_pdf_")
            try:
                try:
                    os.write(fd, uf.getvalue())
                finally:
                    os.close(fd)
...
def main() -> None:
    ...
    # Streamlit Cloud: Secrets → 환경 변수(.env 없을 때)
    try:
        sec = getattr(st, "secrets", None)
        if sec is not None:
            for _k in ("OPENAI_API_KEY", "SUPABASE_URL", "SUPABASE_ANON_KEY"):
                if _k in sec and not os.environ.get(_k):
                    os.environ[_k] = str(sec[_k])
    except Exception:
        pass
